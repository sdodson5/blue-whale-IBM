function s_new = stpm_state_selection(krill_loc, sst_loc, par,s_old, forage_eff,good_whales, sum_calls,south_flag)
...
% Probability of transitioning to S3
sigma_K = (1 + exp(par.state3_pref(1).*(forage_eff - par.state3_pref(2)))).^(-1); % Based on foraging efficiency
...
s3_trans = south_flag.*sigma_K.*sigma_S; % Probability of transition to S3
w1 = (1 - (s3_trans))./(1 + par.w(4));
w2 = par.w(4).*w1;

alpha = par.envir_pref(good_whales,1) + par.envir_pref(good_whales,2).*(sst_loc + par.envir_pref(good_whales,3));
gamma_E = exp(alpha)./(exp(alpha) + 1);  % Environmental influence
gamma_K1 = (1 + exp(par.krill_pref(good_whales,1).*(-krill_loc + par.krill_pref(good_whales,2)))).^(-1); % Krill influence for states 1,2

gamma_K2 = (1 + exp(par.krill_pref(good_whales,1).*(-krill_loc + par.krill_pref(good_whales,3)))).^(-1);  % Krill influence for states 3,4
...
P4_12 = zeros(length(good_whales),1);% Transition to state 4 (south forage)

% Transition rates for whales currently in state 3 or 4
P2_34 = zeros(length(good_whales),1);  % Transition to state 2 (north forage)
P1_34 = zeros(length(good_whales),1);  % Transition to state 1 (north transit)
...
s_new = zeros(length(s_old),1);

for j = 1:length(s_old)
